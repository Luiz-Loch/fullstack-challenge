...
export class ProvablyFair {
  ...
  private static readonly MAX = 2 ** 32;
   ...
   */
  static computeCommitment(serverSeed: string): string {
    return createHash(ProvablyFair.HASH_ALGORITHM).update(serverSeed).digest('hex');
  }
   ...
   */
  static generateCrashPoint(serverSeed: string, clientSeed: string): Multiplier {
    const hash: string = createHmac(ProvablyFair.HASH_ALGORITHM, serverSeed).update(clientSeed).digest('hex');
    return ProvablyFair.hashToCrashPoint(hash);
  }
  ...
  static verify(
    ...
    if (ProvablyFair.computeCommitment(serverSeed) !== commitment) {
      return false;
    }
    const actual: Multiplier = ProvablyFair.generateCrashPoint(serverSeed, clientSeed);
   ...
   */
  private static hashToCrashPoint(hash: string): Multiplier {
     ...
     */
    const raw: number = (1 - ProvablyFair.HOUSE_EDGE) * ProvablyFair.MAX / (ProvablyFair.MAX - h);
